...
DAFTAR_BARANG_PENJUALAN = [
    "Kacang Bawang Renceng Grosir",
    "Kacang Bawang Kiloan"
]
...
def buat_keyboard_barang_penjualan():
    """Buat keyboard pilihan barang penjualan"""
    keyboard = []
    for i, barang in enumerate(DAFTAR_BARANG_PENJUALAN, 1):
        keyboard.append([InlineKeyboardButton(f"{i}. {barang}", callback_data=f"barang_jual_{i}")])
    keyboard.append([InlineKeyboardButton("🚫 Batalkan", callback_data="cancel")])
    return InlineKeyboardMarkup(keyboard)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if callback_data.startswith('menu_'):
        ...
        if menu == 'jual':
            ...
            # Tampilkan statistik
            await tampilkan_statistik(query)
...
"""
        ...
        await query.edit_message_text(
            ...
            parse_mode='Markdown',
            reply_markup=buat_keyboard_barang_penjualan()
        )
...
async def tampilkan_statistik(query):
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == 'input_nama_supplier':
        ...
        try:
            ...
            await proses_pembayaran_message(update, session, nominal_bayar)
            
        except ValueError:
            await update.message.reply_text("❌ Masukkan angka yang valid!")

async def proses_pembayaran_message(update, session, nominal_bayar):
    """Proses pembayaran dari message"""
    total_barang = sum(item['subtotal'] for item in session['data']['daftar_barang'])
    total_retur = sum(item['subtotal'] for item in session['data']['retur_items'])
    total_setelah_retur = total_barang - total_retur
    sisa = nominal_bayar - total_setelah_retur

...
# ===== KONFIGURASI =====
# Ambil token dari environment variable (untuk Railway)
BOT_TOKEN = os.environ.get('BOT_TOKEN', 'YOUR_BOT_TOKEN_HERE')

# Database SQLite (gunakan path yang compatible dengan cloud)
DB_FILE = "/tmp/history_nota.db" if 'RAILWAY_ENVIRONMENT' in os.environ else "history_nota.db"
...
def simpan_histori_nota(user_id, nomor_nota, nama_pelanggan, tanggal_otomatis, daftar_barang, retur_items, total_setelah_retur, bayar, sisa):
    ...
    try:
        ...
        conn.commit()
        conn.close()
        print(f"✅ Nota {nomor_nota} disimpan ke database")
        return True
        
    except Exception as e:
        print(f"❌ Error menyimpan histori: {str(e)}")
        return False
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"📨 Message from {user_id}: {message_text} (state: {user_sessions.get(user_id, {}).get('state', 'idle')})")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"🔄 Callback from {user_id}: {callback_data}")
...
async def buat_dan_kirim_nota(update: Update, context: ContextTypes.DEFAULT_TYPE, data):
    ...
    try:
        ...
        try:
            # Coba load font, fallback ke default jika gagal
            font_large = ImageFont.truetype("/system/fonts/Roboto-Regular.ttf", 20)
            font_medium = ImageFont.truetype("/system/fonts/Roboto-Regular.ttf", 16)
            font_small = ImageFont.truetype("/system/fonts/Roboto-Regular.ttf", 14)
        except:
        ...
        # Header tabel
        draw.text((50, y), "BARANG", fill="black", font=font_medium)
        draw.text((width-100, y), "JUMLAH", fill="black", font=font_medium, anchor="ra")
        y += 25
        
        # Daftar Barang
        for barang in daftar_barang:
            item_text = f"{barang['qty']}x {barang['nama']}"
            draw.text((60, y), item_text, fill="black", font=font_small)
            draw.text((width-60, y), format_jumlah(barang['jumlah']), fill="black", font=font_small, anchor="ra")
            y += 25
        ...
        y += 25
        draw.text((width//2, y), "SUKSES SELALU", fill="black", font=font_small, anchor="mm")
...
def main():
    ...
    # Cek jika token sudah diset
    if BOT_TOKEN == 'YOUR_BOT_TOKEN_HERE':
        print("❌ ERROR: BOT_TOKEN belum diset!")
        print("👉 Untuk Railway: set environment variable BOT_TOKEN")
        print("👉 Untuk local: ganti YOUR_BOT_TOKEN_HERE dengan token bot")
        return
    ...
    print("📍 Environment:", "RAILWAY" if 'RAILWAY_ENVIRONMENT' in os.environ else "LOCAL")
